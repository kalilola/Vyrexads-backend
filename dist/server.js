...
const META_OAUTH_REDIRECT_URI = process.env.FACEBOOK_OAUTH_REDIRECT_URI ||
    process.env.META_OAUTH_REDIRECT_URI ||
    "https://vyrexads-backend.onrender.com/auth/facebook/callback";
const FRONTEND_RETURN_URL = process.env.FRONTEND_RETURN_URL || "http://localhost:8080/analytics";
// =========================================================
...
const META_OAUTH_SCOPES = process.env.FACEBOOK_OAUTH_SCOPES ||
...
if (!API_AUTH_TOKEN)
    console.warn("[env] Missing API_AUTH_TOKEN");
if (!SUPABASE_URL)
    console.warn("[env] Missing SUPABASE_URL");
if (!SUPABASE_SERVICE_ROLE_KEY)
    console.warn("[env] Missing SUPABASE_SERVICE_ROLE_KEY");
// =========================================================
// EXPRESS MIDDLEWARE
// =========================================================
...
const extensionCaptureUpload = (0, multer_1.default)({
...
async function resolveExtensionCaptureOwner(req) {
    if (!supabaseAdmin) {
...
async function relayToN8N(webhookPath, payload, timeoutMs = 60000) {
    if (!N8N_BASE_URL) {
        return {
            status: 500,
...
function relayRoute(webhookPath, timeoutMs = 600000) {
...
async function getMotionAdAttachmentsForSession(owner_id, session_id) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("session_id", `eq.${session_id}`);
    q.set("select", "*");
    q.set("order", "created_at.asc");
    return await supabaseSelect("motion_ad_attachments", q);
}
async function downloadMotionAdAttachment(row) {
    if (!supabaseAdmin) {
        throw new Error("Missing Supabase admin client");
    }
...
async function buildClaudeAttachmentBlocks(rows) {
    const blocks = [];
    if (!rows.length)
        return blocks;
    blocks.push({
        type: "text",
        text: "Voici les assets uploadés par l'utilisateur pour cette publicité motion design. Utilise-les comme base visuelle et ne les ignore pas.",
    });
    for (const row of rows) {
        ...
        if (mimeType.startsWith("video/")) {
            blocks.push({
                type: "text",
                text: `⚠️ VIDÉO DÉTECTÉE : ${fileName}\n` +
        ...
        blocks.push({
            type: "text",
            text: `Fichier uploadé non directement lisible par Claude : ${fileName}\n` +
                `Type : ${mimeType}\n` +
                `URL publique : ${row.public_url || "non disponible"}\n` +
...
function injectAttachmentsIntoLastUserMessage(messages, attachmentBlocks) {
...
app.post("/relay/content-example", requireAuth, relayRoute(N8N_CONTENT_EXAMPLE_WEBHOOK_PATH, 600000));
app.post("/relay/template-regenerate", requireAuth, relayRoute(N8N_TEMPLATE_REGEN_WEBHOOK_PATH, 600000));
app.post("/relay/content-regenerate", requireAuth, relayRoute(N8N_CONTENT_REGENERATE_WEBHOOK_PATH, 600000));
app.post("/relay/content-regenerate-image", requireAuth, relayRoute(N8N_CONTENT_IMAGE_WEBHOOK_PATH, 600000));
app.post("/relay/content-regenerate-carrousel", requireAuth, relayRoute(N8N_CONTENT_CARROUSEL_WEBHOOK_PATH, 600000));
app.post("/relay/content-prompt", requireAuth, relayRoute(N8N_CONTENT_PROMPT_WEBHOOK_PATH, 600000));
app.post("/relay/description-post", requireAuth, relayRoute(N8N_DESCRIPTION_POST_WEBHOOK_PATH, 600000));
app.post("/relay/company-product", requireAuth, relayRoute(N8N_COMPANY_PRODUCT_WEBHOOK_PATH, 600000));
app.post("/relay/competitor-analysis", requireAuth, relayRoute(N8N_COMPETITOR_ANALYSIS_WEBHOOK_PATH, 600000));
function inc(counters, key, amount = 1) {
...
function toBigIntNumber(value, fallback = 0) {
...
function normalizeAccountId(accountIdOrAct) {
    return String(accountIdOrAct || "").replace(/^act_/, "");
}
function act(accountIdOrAct) {
...
function destinationUrlFromCreative(creative) {
...
function primaryTextFromCreative(creative) {
...
function titleFromCreative(creative) {
    const afs = asObject(creative?.asset_feed_spec);
    const oss = asObject(creative?.object_story_spec);
...
function descriptionFromCreative(creative) {
    const afs = asObject(creative?.asset_feed_spec);
    const oss = asObject(creative?.object_story_spec);
    const linkData = asObject(oss?.link_data);
    const videoData = asObject(oss?.video_data);
...
function instagramActorIdFromCreative(creative) {
    const oss = asObject(creative?.object_story_spec);
    return creative?.instagram_actor_id || oss?.instagram_actor_id || null;
}
function callToActionTypeFromCreative(creative) {
    const afs = asObject(creative?.asset_feed_spec);
    const oss = asObject(creative?.object_story_spec);
    const linkData = asObject(oss?.link_data);
    const videoData = asObject(oss?.video_data);
...
function cleanBase64DataUrl(input) {
...
function normalizeMotionQuestionsBlock(text) {
    return String(text || "")
...
function normalizeMotionMetaBlock(text) {
    return String(text || "")
...
function extractMotionMetaBlock(text) {
...
function stripMotionMetaBlock(text) {
    const normalized = normalizeMotionMetaBlock(text);
    return normalized
...
function normalizeMotionDurationSeconds(value) {
    if (value === null || value === undefined || value === "")
        return null;
...
function extractDurationFromMotionCode(code) {
...
function extractMotionDurationSeconds(text, code, fallback) {
    const meta = extractMotionMetaBlock(text);
    const metaDuration = normalizeMotionDurationSeconds(meta?.duration_seconds ??
...
function stripMotionBuilderBlocks(text) {
...
async function deleteMetricRange(table, params) {
    const q = new URLSearchParams();
    ...
    if (params.account_id)
        q.set("account_id", `eq.${params.account_id}`);
    if (params.page_id)
        q.set("page_id", `eq.${params.page_id}`);
    if (params.ig_user_id)
        q.set("ig_user_id", `eq.${params.ig_user_id}`);
    if (params.level)
        q.set("level", `eq.${params.level}`);
    if (params.breakdown_type)
        q.set("breakdown_type", `eq.${params.breakdown_type}`);
...
async function getProviderToken(owner_id, provider) {
    const data = await supabaseRpc("get_provider_token", {
        p_owner_id: owner_id,
        p_provider: provider,
    });
...
function getAccessTokenFromStoredToken(token) {
    let parsed = token;
...
function facebookPageProviderKey(page_id) {
    return `facebook_page:${page_id}`;
}
async function getMetaUserAccessToken(owner_id) {
    const token = await getProviderToken(owner_id, "facebook");
    return getAccessTokenFromStoredToken(token);
}
async function getFacebookPageAccessToken(owner_id, page_id) {
...
async function graphGetAll(path, accessToken, query, maxPages = 50) {
    const firstUrl = new URL(`https://graph.facebook.com/${META_GRAPH_VERSION}/${path.replace(/^\/+/, "")}`);
    for (const [key, value] of Object.entries(query || {})) {
        if (value === undefined || value === null)
            continue;
...
async function graphGetAllSafe(label, path, accessToken, query, maxPages = 50) {
    try {
...
async function graphGetSafe(label, path, accessToken, query) {
    try {
...
// MAPPERS
// =========================================================
function mapAdAccount(owner_id, a) {
...
function mapCampaign(owner_id, account_id, c) {
    return {
        owner_id,
        provider: "facebook",
        account_id,
...
function mapAdSet(owner_id, account_id, s) {
    return {
        owner_id,
        provider: "facebook",
        account_id,
...
function mapAd(owner_id, account_id, ad) {
    return {
        owner_id,
        provider: "facebook",
        account_id,
...
function mapCreative(owner_id, account_id, creative) {
    if (!creative?.id)
        return null;
    return {
        owner_id,
...
function mapIgAccount(owner_id, page_id, ig) {
    return {
        owner_id,
        provider: "facebook",
        ig_user_id: ig.id,
...
function mapPagePost(owner_id, page_id, post) {
    return {
        owner_id,
        provider: "facebook",
        page_id,
...
function mapIgMedia(owner_id, page_id, ig_user_id, media) {
    return {
        owner_id,
        provider: "facebook",
        ig_user_id,
...
function mapInsightBase(owner_id, level, account_id, row) {
...
function mapVideoInsight(owner_id, level, account_id, row) {
    const videoFields = [
        "video_play_actions",
        "video_15_sec_watched_actions",
        "video_30_sec_watched_actions",
...
function mapActionInsights(owner_id, level, account_id, row) {
    const valuesByType = new Map();
    const costsByType = new Map();
    for (const item of asArray(row.action_values)) {
        if (item?.action_type)
            valuesByType.set(item.action_type, toNumber(item.value, 0) || 0);
    }
    for (const item of asArray(row.cost_per_action_type)) {
        if (item?.action_type)
            costsByType.set(item.action_type, toNumber(item.value, 0) || 0);
    }
...
function mapBreakdownInsight(owner_id, level, account_id, breakdownType, row) {
...
// CORE SYNC FUNCTIONS
// =========================================================
async function syncPages(owner_id, userAccessToken, counters, pageIds) {
...
async function syncBusinessesAndCatalogs(owner_id, userAccessToken, counters, includeCatalogs = true) {
    ...
    for (const b of businesses) {
        const catalogs = await graphGetAllSafe(`business_catalogs_${b.id}`, `${b.id}/owned_product_catalogs`, userAccessToken, { fields: "id,name,vertical,business", limit: 100 }, 10);
        ...
        for (const cat of catalogs) {
            const productSets = await graphGetAllSafe(`product_sets_${cat.id}`, `${cat.id}/product_sets`, userAccessToken, { fields: "id,name,filter", limit: 100 }, 10);
            ...
            const products = await graphGetAllSafe(`products_${cat.id}`, `${cat.id}/products`, userAccessToken, {
                fields: [
                    "id",
                    "retailer_id",
                    "name",
...
async function syncOrganicForPages(owner_id, pages, counters) {
    for (const p of pages) {
        ...
        const posts = await graphGetAllSafe(`page_posts_${page_id}`, `${page_id}/posts`, pageToken, { fields: PAGE_POST_FIELDS, limit: 100 }, 20);
        ...
        if (igId) {
            ...
            const media = await graphGetAllSafe(`ig_media_${igId}`, `${igId}/media`, pageToken, { fields: IG_MEDIA_FIELDS, limit: 100 }, 20);
...
async function syncPagePostMetrics(owner_id, page_id, post_id, pageToken, counters) {
    const metrics = [
    ...
    const [postSummary, lifetimeJson] = await Promise.all([
        graphGetSafe(`page_post_summary_${post_id}`, post_id, pageToken, {
            fields: [
                "id",
                "message",
                "story",
        ...
        graphGetSafe(`page_post_lifetime_metrics_${post_id}`, `${post_id}/insights`, pageToken, {
            metric: metrics,
            period: "lifetime",
        }),
    ]);
    ...
    for (const metric of asArray(lifetimeJson?.data)) {
        ...
        if (metric.name === "post_reactions_by_type_total") {
            ...
            if (row.reactions_total === null) {
                row.reactions_total = Object.values(obj).reduce((sum, n) => sum + (toNumber(n, 0) || 0), 0);
            }
        }
...
async function syncInstagramMediaMetrics(owner_id, ig_user_id, media_id, pageToken, counters) {
    ...
    for (const metric of asArray(json.data)) {
        ...
        if (metric.name === "reach")
            row.reach = toBigIntNumber(value, 0);
        if (metric.name === "impressions")
            row.impressions = toBigIntNumber(value, 0);
        if (metric.name === "views")
            row.views = toBigIntNumber(value, 0);
        if (metric.name === "plays")
            row.plays = toBigIntNumber(value, 0);
        if (metric.name === "likes")
            row.likes = toBigIntNumber(value, 0);
        if (metric.name === "comments")
            row.comments = toBigIntNumber(value, 0);
        if (metric.name === "shares")
            row.shares = toBigIntNumber(value, 0);
        if (metric.name === "saved")
            row.saves = toBigIntNumber(value, 0);
        if (metric.name === "replies")
            row.replies = toBigIntNumber(value, 0);
        if (metric.name === "profile_visits")
            row.profile_visits = toBigIntNumber(value, 0);
        if (metric.name === "follows")
            row.follows = toBigIntNumber(value, 0);
        if (metric.name === "total_interactions")
            row.total_interactions = toBigIntNumber(value, 0);
        if (metric.name === "navigation")
            row.navigation = asObject(value);
    }
...
async function syncAdAccounts(owner_id, userAccessToken, counters, accountIds) {
...
async function syncAdsMetadataForAccount(owner_id, account_id, userAccessToken, counters) {
    ...
    for (const c of accountCreatives) {
        ...
        if (row)
            creativeMap.set(row.creative_id, row);
    }
    for (const ad of ads) {
...
async function syncAudiencesForAccount(owner_id, account_id, userAccessToken, counters) {
    const accountAct = act(account_id);
    const customAudiences = await graphGetAllSafe(`custom_audiences_${account_id}`, `${accountAct}/customaudiences`, userAccessToken, {
        fields: [
            "id",
            "name",
            "description",
    ...
    const savedAudiences = await graphGetAllSafe(`saved_audiences_${account_id}`, `${accountAct}/saved_audiences`, userAccessToken, { fields: "id,name,description,targeting,time_created,time_updated,run_status", limit: 100 }, 20);
    const rows = [
...
async function syncPixelsAndConversionsForAccount(owner_id, account_id, userAccessToken, counters) {
    const accountAct = act(account_id);
    const pixels = await graphGetAllSafe(`pixels_${account_id}`, `${accountAct}/adspixels`, userAccessToken, {
    ...
    const customConversions = await graphGetAllSafe(`custom_conversions_${account_id}`, `${accountAct}/customconversions`, userAccessToken, {
...
async function syncLeadFormsAndLeads(owner_id, pages, counters) {
    for (const p of pages) {
        const page_id = p.id;
        let pageToken = p.access_token || "";
        if (!pageToken) {
        ...
        const forms = await graphGetAllSafe(`lead_forms_${page_id}`, `${page_id}/leadgen_forms`, pageToken, {
        ...
        for (const f of forms) {
            const leads = await graphGetAllSafe(`leads_${f.id}`, `${f.id}/leads`, pageToken, {
...
async function syncInsightsForAccount(owner_id, account_id, userAccessToken, counters, since, until) {
    const accountAct = act(account_id);
    const levels = ["account", "campaign", "adset", "ad"];
    for (const level of levels) {
        const fields = level === "ad" ? AD_INSIGHT_FIELDS_SAFE : INSIGHT_FIELDS;
        let rows = await graphGetAllSafe(`insights_${level}_${account_id}`, `${accountAct}/insights`, userAccessToken, {
            level,
            fields,
        ...
        if (level === "ad" && rows.length === 0) {
            console.warn(`[meta][fallback] global ad insights failed/empty for ${account_id}. Trying filtered ad batches.`);
            const q = new URLSearchParams();
            ...
            q.set("limit", "1000");
            const ads = await supabaseSelect("meta_ads", q);
            ...
            for (let i = 0; i < adIds.length; i += 50) {
                ...
                const batchRows = await graphGetAllSafe(`insights_ad_filtered_${account_id}_${i}`, `${accountAct}/insights`, userAccessToken, {
                    level: "ad",
            ...
            if (fallbackRows.length === 0) {
                console.warn(`[meta][fallback] filtered ad batches empty for ${account_id}. Trying ad-by-ad insights.`);
                for (const ad of ads) {
                    if (!ad.ad_id)
                        continue;
                    const adRows = await graphGetAllSafe(`insights_single_ad_${ad.ad_id}`, `${ad.ad_id}/insights`, userAccessToken, {
                        fields: AD_INSIGHT_FIELDS_SAFE,
                        time_range: JSON.stringify({ since, until }),
                        time_increment: 1,
                        limit: 500,
...
async function syncBreakdownsForAccount(owner_id, account_id, userAccessToken, counters, since, until, breakdowns) {
    const accountAct = act(account_id);
    ...
    for (const level of levels) {
        for (const breakdownType of breakdowns) {
            ...
            const rows = await graphGetAllSafe(`breakdown_${level}_${breakdownType}_${account_id}`, `${accountAct}/insights`, userAccessToken, {
                level,
...
app.post("/api/screenshot", requireAuth, async (req, res) => {
    ...
    try {
      ...
      `,
        ...
        const fileName = `${Date.now()}-${hostname}-screenshot-${crypto_1.default.randomUUID()}.jpg`;
...
// =========================================================
app.post("/api/extension/captures", extensionCaptureUpload.fields([
    { name: "png", maxCount: 1 },
    ...
    try {
        ...
        const project_id = String(req.body?.project_id ||
            req.body?.projectId ||
            "").trim();
        const requestedSessionId = String(req.body?.session_id ||
            req.body?.sessionId ||
            "").trim();
        const rawMetadata = parseJsonBodyField(req.body?.metadata);
        const metadata = {
            ...rawMetadata,
        ...
        const captureId = crypto_1.default.randomUUID();
        const safeHost = sanitizeStorageSegment(metadata?.page_url ? new URL(metadata.page_url).hostname : "page");
        const basePath = [
            owner_id,
            "extension-captures",
...
app.get("/auth/facebook/start", async (req, res) => {
    try {
        if (!META_APP_ID)
            throw new Error("Missing META_APP_ID / FACEBOOK_APP_ID");
...
app.get("/auth/facebook/callback", async (req, res) => {
    ...
    try {
        ...
        if (me?.id)
            u.searchParams.set("facebook_user_id", String(me.id));
...
app.post("/api/meta/debug/post-insights", requireAuth, async (req, res) => {
    try {
        ...
        if (!post_id)
            return res.status(400).json({ ok: false, error: "Missing post_id" });
        const since = String(req.body?.since || "").trim();
        const until = String(req.body?.until || "").trim();
        const metrics = Array.isArray(req.body?.metrics) && req.body.metrics.length
...
app.post("/api/meta/debug/post-metrics-matrix", requireAuth, async (req, res) => {
    try {
        ...
        if (!page_id)
            return res.status(400).json({ ok: false, error: "Missing page_id" });
        if (!post_ids.length)
            return res.status(400).json({ ok: false, error: "Missing post_ids" });
        const metrics = [
            "post_impressions",
            "post_impressions_unique",
...
app.post("/api/meta/debug/page-posts", requireAuth, async (req, res) => {
    try {
        ...
        const [posts, publishedPosts, feed] = await Promise.all([
            graphGetAllSafe(`debug_posts_${page_id}`, `${page_id}/posts`, pageToken, { fields: PAGE_POST_FIELDS, limit: 25 }, 5),
            graphGetAllSafe(`debug_published_posts_${page_id}`, `${page_id}/published_posts`, pageToken, { fields: PAGE_POST_FIELDS, limit: 25 }, 5),
            graphGetAllSafe(`debug_feed_${page_id}`, `${page_id}/feed`, pageToken, { fields: PAGE_POST_FIELDS, limit: 25 }, 5),
        ]);
        return res.json({
            ok: true,
            page_id,
...
const TIKTOK_OAUTH_SCOPES = process.env.TIKTOK_OAUTH_SCOPES ||
...
function normalizeTikTokToken(raw, owner_id) {
    let parsed = raw;
    if (typeof parsed === "string") {
        try {
            parsed = JSON.parse(parsed);
...
function isTikTokTokenExpiringSoon(value, minutes = 10) {
    if (!value || typeof value !== "string")
        return false;
...
function extractTikTokOAuthPayload(raw) {
    return raw?.data && typeof raw.data === "object" ? raw.data : raw;
}
function tiktokErrorMessage(json) {
...
function toTikTokIsoFromCreateTime(value) {
    const n = toNumber(value, null);
    if (n === null)
        return null;
    return new Date(n * 1000).toISOString();
}
function safeRate(delta, previous) {
    if (delta === null || previous === null || previous <= 0)
        return null;
    return delta / previous;
}
async function getTikTokStoredToken(owner_id) {
    const token = await getProviderToken(owner_id, "tiktok");
    return normalizeTikTokToken(token, owner_id);
}
async function refreshTikTokTokenIfNeeded(owner_id, token) {
...
async function getFreshTikTokToken(owner_id) {
...
async function fetchTikTokUserInfo(access_token) {
    try {
...
async function fetchTikTokVideoList(access_token, options) {
...
async function fetchTikTokVideoQuery(access_token, video_ids) {
...
async function selectTikTokProfile(owner_id, open_id) {
    const q = new URLSearchParams();
    q.set("select", "id,owner_id,open_id,display_name,username,last_synced_at");
    q.set("owner_id", `eq.${owner_id}`);
    q.set("open_id", `eq.${open_id}`);
    q.set("limit", "1");
    const rows = await supabaseSelect("tiktok_profiles", q);
    return rows[0] || null;
}
async function selectLatestTikTokProfileSnapshot(owner_id, open_id) {
    const q = new URLSearchParams();
    ...
    q.set("limit", "1");
    const rows = await supabaseSelect("tiktok_profile_metric_snapshots", q);
    return rows[0] || null;
}
async function selectLatestTikTokVideoSnapshot(owner_id, video_id) {
    const q = new URLSearchParams();
    ...
    q.set("order", "snapshot_at.desc");
    q.set("limit", "1");
    const rows = await supabaseSelect("tiktok_video_metric_snapshots", q);
    return rows[0] || null;
}
async function selectTikTokVideoIdMap(owner_id, video_ids) {
    ...
    q.set("owner_id", `eq.${owner_id}`);
    q.set("video_id", `in.(${cleanIds.map((id) => `"${id.replace(/"/g, "")}"`).join(",")})`);
    const rows = await supabaseSelect("tiktok_videos", q);
    return new Map(rows
        .filter((r) => r?.video_id && r?.id)
        .map((r) => [String(r.video_id), String(r.id)]));
}
function mapTikTokProfileRow(owner_id, user) {
    return {
        owner_id,
        provider: "tiktok",
    ...
    await supabaseInsert("tiktok_profile_metric_snapshots", [
        {
            ...
            follower_growth_rate: safeRate(followerDelta, previous ? toBigIntNumber(previous.follower_count, 0) : null),
            likes_growth_rate: safeRate(likesDelta, previous ? toBigIntNumber(previous.likes_count, 0) : null),
...
async function upsertTikTokSyncState(params) {
    await supabaseUpsert("tiktok_sync_state", [
        {
            owner_id: params.owner_id,
...
async function syncTikTokProfileToDb(owner_id) {
    ...
    try {
        ...
        await supabaseUpsert("tiktok_profiles", [profileRow], "owner_id,provider");
...
async function syncTikTokVideosToDb(owner_id, options = {}) {
    const startedAt = new Date().toISOString();
    ...
    try {
        ...
        for (let page = 0; page < maxPages; page++) {
            ...
            const upserted = await supabaseUpsert("tiktok_videos", videoRows, "owner_id,video_id");
            totalVideosUpserted += upserted;
            const videoIds = videos.map((v) => String(v.id || "")).filter(Boolean);
...
async function syncTikTokVideoQueryToDb(owner_id, video_ids) {
    const startedAt = new Date().toISOString();
    try {
        const token = await getFreshTikTokToken(owner_id);
        const profileSync = await syncTikTokProfileToDb(owner_id);
...
app.get("/auth/tiktok/callback", async (req, res) => {
    ...
    try {
        ...
        if (!code_verifier)
            throw new Error("Missing TikTok code_verifier");
        ...
        if (profileResult?.profile?.id)
            u.searchParams.set("tiktok_profile_id", String(profileResult.profile.id));
...
app.post("/api/tiktok/sync/videos", requireAuth, async (req, res) => {
    try {
        ...
        const result = await syncTikTokVideosToDb(owner_id, {
            cursor: req.body?.cursor !== undefined && req.body?.cursor !== null
...
app.post("/api/tiktok/sync/video-query", requireAuth, async (req, res) => {
    try {
        ...
        const video_ids = Array.isArray(req.body?.video_ids)
            ? req.body.video_ids.map(String).map((s) => s.trim()).filter(Boolean)
            : [];
...
const GOOGLE_ADS_OAUTH_REDIRECT_URI = process.env.GOOGLE_ADS_OAUTH_REDIRECT_URI ||
    process.env.GOOGLE_OAUTH_REDIRECT_URI ||
    "https://vyrexads-backend.onrender.com/auth/google-ads/callback";
const GOOGLE_ADS_DEFAULT_LOGIN_CUSTOMER_ID = cleanGoogleAdsCustomerId(process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID || "");
if (!GOOGLE_ADS_DEVELOPER_TOKEN)
    console.warn("[env] Missing GOOGLE_ADS_DEVELOPER_TOKEN");
if (!GOOGLE_ADS_CLIENT_ID)
    console.warn("[env] Missing GOOGLE_ADS_CLIENT_ID");
if (!GOOGLE_ADS_CLIENT_SECRET)
    console.warn("[env] Missing GOOGLE_ADS_CLIENT_SECRET");
...
function googleAdsArray(value) {
    return Array.isArray(value) ? value : [];
}
function firstDefined(...values) {
    return values.find((v) => v !== undefined && v !== null && v !== "") ?? null;
}
function getGoogleAdsIdFromResource(resourceName) {
    const s = String(resourceName || "");
    const parts = s.split("/");
    return parts[parts.length - 1] || null;
}
function googleAdsErrorMessage(payload) {
...
function normalizeGoogleAdsToken(token) {
    let parsed = token;
    if (typeof parsed === "string") {
        try {
            parsed = JSON.parse(parsed);
...
function isGoogleAccessTokenExpired(token) {
    ...
    return ms <= Date.now() + 120000;
}
async function refreshGoogleAdsToken(owner_id, token) {
    assertGoogleAdsEnv();
...
function jsonbStripNullsLocal(obj) {
    const out = {};
...
async function getGoogleAdsAccessToken(owner_id) {
...
async function googleAdsListAccessibleCustomers(owner_id) {
    assertGoogleAdsEnv();
...
function googleCampaignTypeFlags(campaign) {
...
function googleAdCreativeFields(ad) {
    return {
        ...
        youtube_video_id: firstDefined(ad?.videoAd?.video?.youtubeVideoId, ad?.videoResponsiveAd?.videos?.[0]?.youtubeVideoId),
        youtube_video_title: firstDefined(ad?.videoAd?.video?.youtubeVideoTitle, ad?.videoResponsiveAd?.videos?.[0]?.youtubeVideoTitle),
...
async function syncGoogleAdsCampaignMetrics(params) {
    ...
    return supabaseUpsert("ads_metrics_campaigns", mapped, "owner_id,provider,customer_id,campaign_id,date");
}
async function syncGoogleAdsAdGroupMetrics(params) {
    ...
    return supabaseUpsert("ads_metrics_ad_groups", mapped, "owner_id,provider,customer_id,ad_group_id,date");
}
async function syncGoogleAdsAdMetrics(params) {
    ...
    return supabaseUpsert("ads_metrics_ads", mapped, "owner_id,provider,customer_id,ad_id,date");
}
async function syncGoogleAdsKeywordMetrics(params) {
    ...
    return supabaseUpsert("ads_metrics_keywords", mapped, "owner_id,provider,customer_id,ad_group_id,criterion_id,date");
}
function googleAdsStableUuid(...parts) {
    const input = parts.map((p) => String(p ?? "")).join("|");
    const h = crypto_1.default.createHash("sha256").update(input).digest("hex");
...
function addStableGoogleAdsId(table, row, ...parts) {
    return {
        id: googleAdsStableUuid(table, ...parts),
        ...row,
    };
}
async function googleAdsUpsertById(table, rows) {
    const deduped = Array.from(new Map(rows
        .filter((row) => row?.id)
        .map((row) => [String(row.id), row])).values());
    return supabaseUpsert(table, deduped, "id");
}
async function optionalGoogleAdsSync(label, fn) {
    try {
        return await fn();
    }
    catch (e) {
...
function googleAdsChangedFields(value) {
    if (Array.isArray(value))
...
function googleAdsCriterionFields(scope, r) {
    ...
    return addStableGoogleAdsId("google_ads_criteria", {
        owner_id: "",
        provider: "google_ads",
        customer_id: "",
        scope,
        ...
        raw_json: r,
        last_synced_at: new Date().toISOString(),
    }, scope, c.id, ag.id, prefix.criterionId);
}
...
async function syncGoogleAdsConversionActions(params) {
    ...
    return supabaseUpsert("google_ads_conversion_actions", mapped, "owner_id,customer_id,conversion_action_id");
}
...
function googleAdsAssetLinkRow(params) {
    ...
    const link = asObject(r?.customerAsset ||
        r?.campaignAsset ||
        r?.adGroupAsset ||
    ...
    return addStableGoogleAdsId("google_ads_asset_links", {
        owner_id: params.owner_id,
        provider: "google_ads",
        customer_id: cleanGoogleAdsCustomerId(params.customer_id),
        ...
        raw_json: r,
        last_synced_at: new Date().toISOString(),
    }, params.owner_id, params.customer_id, params.link_scope, campaignId, adGroupId, adId, assetGroupId, assetId, fieldType);
}
...
async function syncGoogleAdsAssetGroupAssets(params) {
    ...
    const mapped = rows.map((r) => {
        ...
        return addStableGoogleAdsId("google_ads_asset_group_assets", {
            owner_id: params.owner_id,
            provider: "google_ads",
            customer_id: cleanGoogleAdsCustomerId(params.customer_id),
            campaign_id: c.id ? String(c.id) : null,
            ...
            raw_json: r,
            last_synced_at: new Date().toISOString(),
        }, params.owner_id, params.customer_id, ag.id, assetId, aga.fieldType);
...
async function syncGoogleAdsAssetGroupSignals(params) {
    ...
    const mapped = rows.map((r) => {
        ...
        return addStableGoogleAdsId("google_ads_asset_group_signals", {
            owner_id: params.owner_id,
            provider: "google_ads",
            customer_id: cleanGoogleAdsCustomerId(params.customer_id),
            campaign_id: c.id ? String(c.id) : null,
            ...
            raw_json: r,
            last_synced_at: new Date().toISOString(),
        }, params.owner_id, params.customer_id, ag.id, signalId, s?.searchTheme?.text);
...
async function syncGoogleAdsAssetGroupTopCombinations(params) {
    ...
    const mapped = rows.map((r) => {
        ...
        return addStableGoogleAdsId("google_ads_asset_group_top_combinations", {
            owner_id: params.owner_id,
            provider: "google_ads",
            customer_id: cleanGoogleAdsCustomerId(params.customer_id),
            campaign_id: c.id ? String(c.id) : null,
            ...
            raw_json: r,
            last_synced_at: new Date().toISOString(),
        }, params.owner_id, params.customer_id, c.id, ag.id, JSON.stringify(combinations));
...
async function syncGoogleAdsSearchTermMetrics(params) {
    ...
    const structureRows = baseRows.map((r) => addStableGoogleAdsId("google_ads_search_terms", {
        owner_id: r.owner_id,
        provider: r.provider,
    ...
    }, r.owner_id, r.customer_id, r.campaign_id, r.ad_group_id, r.search_term));
    const metricRows = baseRows.map((r) => addStableGoogleAdsId("ads_metrics_search_terms", {
        owner_id: r.owner_id,
        provider: r.provider,
        customer_id: r.customer_id,
        campaign_id: r.campaign_id,
        ...
        raw: r.raw,
    }, r.owner_id, r.customer_id, r.campaign_id, r.ad_group_id, r.search_term, r.date));
...
async function syncGoogleAdsLandingPageMetrics(params) {
    ...
    const structureRows = baseRows.map((r) => addStableGoogleAdsId("google_ads_landing_pages", {
        owner_id: r.owner_id,
        provider: r.provider,
        customer_id: r.customer_id,
        campaign_id: r.campaign_id,
        ...
        last_seen_at: r.last_seen_at,
        last_synced_at: r.last_synced_at,
    }, r.owner_id, r.customer_id, r.campaign_id, r.ad_group_id, r.final_url));
    const metricRows = baseRows.map((r) => addStableGoogleAdsId("ads_metrics_landing_pages", {
        owner_id: r.owner_id,
        provider: r.provider,
        customer_id: r.customer_id,
        campaign_id: r.campaign_id,
        ...
        conversion_value: r.conversion_value,
        raw: r.raw,
    }, r.owner_id, r.customer_id, r.campaign_id, r.ad_group_id, r.final_url, r.date));
...
async function syncGoogleAdsAssetMetrics(params) {
    ...
    const mapped = rows.map(({ r, scope }) => {
        ...
        const seg = googleSegments(r);
        const assetId = String(asset.id || getGoogleAdsIdFromResource(link.asset) || "");
        return addStableGoogleAdsId("ads_metrics_assets", {
            owner_id: params.owner_id,
            provider: "google_ads",
            customer_id: cleanGoogleAdsCustomerId(params.customer_id),
            campaign_id: c.id ? String(c.id) : null,
            ...
            conversion_value: toNumber(metrics.conversionsValue, null),
            raw: r,
        }, params.owner_id, params.customer_id, scope, c.id, ag.id, assetId, link.fieldType, seg.date);
...
async function syncGoogleAdsShoppingProductMetrics(params) {
    ...
    const structureRows = baseRows.map((r) => addStableGoogleAdsId("google_ads_shopping_products", {
        owner_id: r.owner_id,
        provider: r.provider,
        customer_id: r.customer_id,
        campaign_id: r.campaign_id,
        ...
        last_seen_at: r.last_seen_at,
        last_synced_at: r.last_synced_at,
    }, r.owner_id, r.customer_id, r.product_item_id, r.campaign_id, r.ad_group_id, r.asset_group_id));
    const metricRows = baseRows.map((r) => addStableGoogleAdsId("ads_metrics_shopping_products", {
        owner_id: r.owner_id,
        provider: r.provider,
        customer_id: r.customer_id,
        campaign_id: r.campaign_id,
        ...
        raw: r.raw,
    }, r.owner_id, r.customer_id, r.product_item_id, r.campaign_id, r.ad_group_id, r.asset_group_id, r.date));
...
async function syncGoogleAdsChangeEvents(params) {
    ...
    const safeDateFrom = params.date_from && params.date_from > minChangeEventDateStr
        ? params.date_from
        : minChangeEventDateStr;
    const safeDateTo = params.date_to && params.date_to < todayStr
        ? params.date_to
        : todayStr;
    const rows = await tryGoogleAdsSearchStream({
        owner_id: params.owner_id,
...
async function resolveGoogleAdsCustomerIds(body, owner_id) {
    const requested = [
        ...(body.customer_ids || []),
    ...
    const requestedLoginCustomerId = cleanGoogleAdsCustomerId(body.login_customer_id || GOOGLE_ADS_DEFAULT_LOGIN_CUSTOMER_ID);
...
async function getGoogleAdsLoginCustomerIdFor(owner_id, customer_id, fallback) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("provider", "eq.google_ads");
    ...
    q.set("limit", "1");
    const rows = await supabaseSelect("google_ads_accounts", q);
    const row = rows[0];
    return cleanGoogleAdsCustomerId(row?.login_customer_id ||
    ...
    try {
        ...
        for (const customer_id of accessible) {
            const result = await syncGoogleAdsAccountRows({
                owner_id,
                customer_id,
                login_customer_id: await getGoogleAdsLoginCustomerIdFor(owner_id, customer_id, login_customer_id || customer_id),
            });
    ...
    try {
        ...
        for (const customer_id of customerIds) {
            try {
                ...
                for (const level of ["campaign", "ad_group", "ad", "keyword", "search_term", "landing_page", "asset", "asset_group", "shopping"]) {
                ...
                inc(counters, "change_events_synced", await optionalGoogleAdsSync("change_events", async () => syncGoogleAdsChangeEvents({
                    owner_id,
                    customer_id,
                    login_customer_id: await getGoogleAdsLoginCustomerIdFor(owner_id, customer_id, login_customer_id || customer_id),
                    date_from,
...
if (!ANTHROPIC_API_KEY)
    console.warn("[env] Missing ANTHROPIC_API_KEY");
const ANTHROPIC_MODEL = process.env.ANTHROPIC_MODEL || "claude-fable-5";
const ANTHROPIC_COMPANY_MODEL = process.env.ANTHROPIC_COMPANY_MODEL || ANTHROPIC_MODEL;
...
function normalizeCompanyMarketingAngles(value) {
    return asArray(value)
        ...
        return {
            ...
            targetClient: String(raw.targetClient ?? raw.target_client ?? raw.client ?? raw.segment ?? "").trim(),
        };
    })
        .filter((item) => item.name || item.description);
}
function normalizeCompanyTargetClientBenefits(value) {
    return asArray(value)
        .map((item) => {
        const raw = asObject(item);
        return {
            ...
            initialProblem: String(raw.initialProblem ?? raw.initial_problem ?? raw.problem ?? "").trim(),
            concreteBenefit: String(raw.concreteBenefit ?? raw.concrete_benefit ?? raw.benefit ?? "").trim(),
        };
    })
        .filter((item) => item.targetClient ||
        item.description ||
        item.initialProblem ||
...
app.post("/api/company/marketing-angle-suggestions", requireAuth, async (req, res) => {
    try {
        ...
        const existingMarketingAngles = normalizeCompanyMarketingAngles(company.existingMarketingAngles ?? company.marketingAngles ?? []);
        const targetClientBenefits = normalizeCompanyTargetClientBenefits(company.targetClientBenefits ?? []);
...
${JSON.stringify(contextForClaude, null, 2)}`;
        ...
        let marketingAngles = normalizeCompanyMarketingAngles(parsed.marketingAngles ?? parsed.marketing_angles ?? parsed.suggestions ?? []);
        let generatedTargetClientBenefits = normalizeCompanyTargetClientBenefits(parsed.targetClientBenefits ?? parsed.target_client_benefits ?? []);
        ...
        marketingAngles = marketingAngles.map((angle) => ({
            ...angle,
            targetClient: angle.targetClient && allowedTargets.has(angle.targetClient)
                ? angle.targetClient
                : "",
        }));
        return res.json({
...
async function getActiveMotionAdMessages(owner_id, session_id) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("session_id", `eq.${session_id}`);
    ...
    return await supabaseSelect("motion_ad_messages", q);
}
async function getActiveMotionAdMessageAtPosition(owner_id, session_id, position) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("session_id", `eq.${session_id}`);
    q.set("is_active", "eq.true");
    ...
    q.set("select", "*");
    q.set("limit", "1");
    const rows = await supabaseSelect("motion_ad_messages", q);
    return rows[0] || null;
}
async function getNextMotionAdMessagePosition(owner_id, session_id) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("session_id", `eq.${session_id}`);
    q.set("is_active", "eq.true");
...
async function deactivateMotionAdMessagesFromPosition(owner_id, session_id, position) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("session_id", `eq.${session_id}`);
    q.set("is_active", "eq.true");
...
function motionDbMessagesToAnthropicMessages(rows) {
    return rows
        ...
        const content = row.role === "assistant" && code
            ? `${text}\n\n[CODE]\n${code}\n[/CODE]`.trim()
            : text;
        return {
...
function normalizeMotionContextSelection(value) {
    const raw = asObject(value);
    const ids = (v) => asArray(v).map((item) => String(item || "").trim()).filter(Boolean);
    return {
        ...raw,
        personas: ids(raw.personas),
        solutions: ids(raw.solutions),
        needs: ids(raw.needs),
        painPoints: ids(raw.painPoints ?? raw.pain_points),
        includeCompetitors: Boolean(raw.includeCompetitors ?? raw.include_competitors),
        competitors: ids(raw.competitors),
        selected: asObject(raw.selected),
    };
}
function motionContextOption(prefix, label, sub, source, payload) {
    const cleanLabel = String(label || "").trim();
    if (!cleanLabel)
        return null;
    const cleanSub = String(sub || "").trim();
    const id = `${prefix}_${hashJson({ label: cleanLabel, source, payload }).slice(0, 14)}`;
    return {
        id,
        label: cleanLabel,
        sub: cleanSub,
...
function pushUniqueMotionContextOption(target, option) {
    if (!option)
        return;
    const key = `${option.source}::${option.label}`.toLowerCase();
    if (target.some((item) => `${item.source}::${item.label}`.toLowerCase() === key))
        return;
    target.push(option);
}
...
function arrayFromPossibleFields(row, keys) {
    const out = [];
...
function compactCompanyForMotionContext(company) {
    ...
    for (const [key, value] of Object.entries(raw)) {
        ...
        if (blockedKeys.has(key) || lower.includes("social") || lower.includes("réseau") || lower.includes("reseau")) {
            continue;
        }
        if (value === null || value === undefined || value === "")
            continue;
...
async function safeSupabaseSelect(table, query, label) {
    try {
        return await supabaseSelect(table, query);
    }
    catch (e) {
        console.warn(`[motion-ad][context][skip] ${label || table}:`, e?.message || String(e));
        return [];
    }
}
async function selectRowsForOwnerCompany(table, owner_id, company_id) {
    if (company_id) {
        const byCompany = new URLSearchParams();
        byCompany.set("owner_id", `eq.${owner_id}`);
        byCompany.set("company_id", `eq.${company_id}`);
        byCompany.set("select", "*");
        byCompany.set("limit", "200");
        const rows = await safeSupabaseSelect(table, byCompany, `${table} by company_id`);
        if (rows.length > 0)
            return rows;
    }
    const byOwner = new URLSearchParams();
    byOwner.set("owner_id", `eq.${owner_id}`);
    byOwner.set("select", "*");
    byOwner.set("limit", "200");
    const rows = await safeSupabaseSelect(table, byOwner, `${table} by owner_id`);
    if (!company_id)
        return rows;
    return rows.filter((row) => {
        const rowCompanyId = row?.company_id || row?.companyId || row?.entity_id || row?.entityId;
        return !rowCompanyId || String(rowCompanyId) === String(company_id);
    });
}
async function getMotionAdSession(owner_id, session_id) {
    const q = new URLSearchParams();
    q.set("owner_id", `eq.${owner_id}`);
    q.set("id", `eq.${session_id}`);
    q.set("select", "*");
    q.set("limit", "1");
    const rows = await safeSupabaseSelect("motion_ad_sessions", q, "motion_ad_sessions");
    return rows[0] || null;
}
async function buildMotionAdCompanyStrategyContext(params) {
    ...
    if (effectiveCompanyId) {
        ...
        cq.set("select", "*");
        cq.set("limit", "1");
        const companies = await safeSupabaseSelect("companies", cq, "companies by id");
    ...
    if (!company) {
        ...
        cq.set("select", "*");
        cq.set("limit", "1");
        const companies = await safeSupabaseSelect("companies", cq, "companies by owner_id");
        company = companies[0] || null;
        effectiveCompanyId = company?.id ? String(company.id) : effectiveCompanyId;
    }
    const uploadedFilesQuery = new URLSearchParams();
    uploadedFilesQuery.set("owner_id", `eq.${owner_id}`);
    uploadedFilesQuery.set("select", "*");
    uploadedFilesQuery.set("limit", "80");
    uploadedFilesQuery.set("order", "created_at.desc");
    if (effectiveCompanyId)
        uploadedFilesQuery.set("entity_id", `eq.${effectiveCompanyId}`);
    uploadedFilesQuery.set("context", "in.(company_logo,company_visual,company_info,company_font,company_product,company_charte,company_charter,brand_charter,brand_asset)");
    const companyFiles = await safeSupabaseSelect("uploaded_files", uploadedFilesQuery, "uploaded_files");
    ...
    const companyTargetBenefits = arrayFromPossibleFields(companyContext, [
        ...
        "clients_cibles_problemes_benefices",
    ]);
    for (const item of companyTargetBenefits) {
        const raw = asObject(item);
        const label = raw.targetClient || raw.target_client || raw.client || raw.segment || raw.name || raw.title;
        const sub = raw.description || raw.initialProblem || raw.initial_problem || raw.concreteBenefit || raw.concrete_benefit;
        pushUniqueMotionContextOption(options.personas, motionContextOption("persona", label, sub, "company.targetClientBenefits", raw));
        pushUniqueMotionContextOption(options.painPoints, motionContextOption("pain", raw.initialProblem || raw.initial_problem, label, "company.targetClientBenefits.initialProblem", raw));
    ...
    const companyMarketingAngles = arrayFromPossibleFields(companyContext, [
        ...
        "positioning",
        "positionnement",
        "subProblemsMarketingAngles",
        "sub_problems_marketing_angles",
    ]);
    for (const item of companyMarketingAngles) {
        const raw = asObject(item);
        const label = raw.name || raw.title || raw.label || raw.angle || raw.value;
        const sub = raw.description || raw.problem || raw.targetClient || raw.target_client;
        pushUniqueMotionContextOption(options.solutions, motionContextOption("solution", label, sub, "company.marketingAngles", raw));
    }
    for (const target of strategyTargets) {
        const rawTarget = asObject(target);
        const targetLabel = rawTarget.name || rawTarget.title || rawTarget.label || rawTarget.description;
        const targetSub = rawTarget.description || rawTarget.awarenessLevel || rawTarget.awareness_level;
        pushUniqueMotionContextOption(options.personas, motionContextOption("target", targetLabel, targetSub, "strategy_targets", rawTarget));
        const personas = arrayFromPossibleFields(rawTarget, ["personas", "persona", "target_personas"]);
        for (const persona of personas) {
            const raw = asObject(persona);
            const label = raw.targetClient || raw.target_client || raw.name || raw.profession || raw.segment || targetLabel;
            const sub = [raw.profession, raw.age, raw.location, raw.initialProblem || raw.initial_problem]
                .filter(Boolean)
                .join(" · ");
            pushUniqueMotionContextOption(options.personas, motionContextOption("persona", label, sub, "strategy_targets.personas", { target: rawTarget, persona: raw }));
        }
        const solutionItems = [
            ...arrayFromPossibleFields(rawTarget, [
                "solutions",
                "marketingAngles",
                "marketing_angles",
                "subProblemsMarketingAngles",
                "sub_problems_marketing_angles",
                "Sous-problèmes résolus / angles marketing",
                "sousProblemesResolusAnglesMarketing",
            ]),
        ];
        for (const item of solutionItems) {
            const raw = typeof item === "string" ? { name: item } : asObject(item);
            pushUniqueMotionContextOption(options.solutions, motionContextOption("solution", raw.name || raw.title || raw.label || raw.value, raw.description || targetLabel, "strategy_targets.solutions", { target: rawTarget, item: raw }));
        }
        for (const item of arrayFromPossibleFields(rawTarget, ["needsMotivations", "needs_motivations", "needs", "motivations"])) {
            const raw = typeof item === "string" ? { name: item } : asObject(item);
            pushUniqueMotionContextOption(options.needs, motionContextOption("need", raw.name || raw.title || raw.label || raw.value, raw.description || targetLabel, "strategy_targets.needsMotivations", { target: rawTarget, item: raw }));
        }
        for (const item of arrayFromPossibleFields(rawTarget, ["painPoints", "pain_points", "problems", "problematiques", "objections"])) {
            const raw = typeof item === "string" ? { name: item } : asObject(item);
            pushUniqueMotionContextOption(options.painPoints, motionContextOption("pain", raw.name || raw.title || raw.label || raw.value, raw.description || targetLabel, "strategy_targets.painPoints", { target: rawTarget, item: raw }));
        }
    }
    for (const competitor of strategyCompetitors) {
        const raw = asObject(competitor);
        const label = raw.name || raw.title || raw.competitor_name || raw.company_name || raw.website || raw.url;
        const sub = raw.tagline || raw.description || raw.website || raw.url || raw.positioning;
        pushUniqueMotionContextOption(options.competitors, motionContextOption("competitor", label, sub, "strategy_competitors", raw));
        for (const item of arrayFromPossibleFields(raw, ["competitors", "items", "data"])) {
            const nested = asObject(item);
            pushUniqueMotionContextOption(options.competitors, motionContextOption("competitor", nested.name || nested.title || nested.website || nested.url, nested.description || nested.tagline || nested.website || nested.url, "strategy_competitors.items", nested));
        }
    }
    const optionById = new Map();
    for (const section of ["personas", "solutions", "needs", "painPoints", "competitors"]) {
        for (const option of options[section])
            optionById.set(option.id, option);
    }
    const selectById = (ids) => ids.map((id) => optionById.get(id)).filter(Boolean);
    const selectedFromFront = asObject(selection.selected);
    const selected = {
        personas: selectById(selection.personas || []).concat(asArray(selectedFromFront.personas)),
        solutions: selectById(selection.solutions || []).concat(asArray(selectedFromFront.solutions)),
        needs: selectById(selection.needs || []).concat(asArray(selectedFromFront.needs)),
        painPoints: selectById(selection.painPoints || selection.pain_points || []).concat(asArray(selectedFromFront.painPoints || selectedFromFront.pain_points)),
        competitors: (selection.includeCompetitors || selection.include_competitors)
            ? selectById(selection.competitors || []).concat(asArray(selectedFromFront.competitors))
            : [],
    };
    return {
        ...
        strategy: {
            ...
            objectives_smart: strategyObjectives,
            competitors_observed: strategyCompetitors,
        },
        options,
        selection,
        selected,
...
function buildMotionAdSystemContextAppendix(context) {
    const json = JSON.stringify(context, null, 2).slice(0, 120000);
    return `

============================================================
INFORMATIONS ENTREPRISE & STRATÉGIE VYREXADS
============================================================

Tu reçois ci-dessous le contexte entreprise et stratégie récupéré depuis la base de données Vyrexads.
Tu dois l'utiliser comme source prioritaire pour comprendre le SaaS, l'offre, la cible, la charte et le positionnement.

Règles d'utilisation :
- Utilise la page Identité entreprise sauf les réseaux sociaux.
- Utilise la charte, les fichiers et URLs d'assets de marque quand ils sont présents.
- Pour le positionnement de l'offre, privilégie uniquement les éléments sélectionnés dans l'overlay : solutions, douleurs, besoins, personas.
- Pour la stratégie, utilise le ciblage sélectionné dans l'overlay : personas, solutions proposées, besoins / motivations et problèmes / douleurs.
- Les Objectifs SMART peuvent guider le message et le CTA.
- Si des concurrents observés existent mais ne sont pas sélectionnés dans l'overlay, ne positionne pas l'ad contre eux automatiquement : demande d'abord si l'utilisateur veut les prendre en compte.
- Si des concurrents sont sélectionnés, tu peux positionner l'ad face à eux et mettre en avant les différenciateurs.
- Si une donnée manque ou est vide, fais une hypothèse simple et indique-la dans [RÉFLEXION].

Contexte JSON Vyrexads :
${json}`;
}
...
app.post("/api/motion-ad/chat", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const context_company_id = motionCompanyStrategyContext.company_id || requestedContextCompanyId;
...
CONTRAINTE ACTIVE DE CETTE REQUÊTE : format demandé = ${requested_ad_format || "non précisé"}, durée demandée = ${requested_motion_duration || requested_duration_seconds || "non précisée"} secondes. Le bloc [META], const DURATION et window.__VYREXADS_DURATION__ doivent utiliser cette durée exacte si elle est précisée.`;
            ...
            await supabaseUpsert("motion_ad_sessions", [
                {
                    id: session_id,
                    owner_id,
                    ...(context_company_id ? { company_id: context_company_id } : {}),
                    ...(["9:16", "1:1", "16:9", "4:5"].includes(requested_ad_format) ? { ad_format: requested_ad_format } : {}),
                    updated_at: new Date().toISOString(),
                },
            ], "id");
        }
        let userMessageId = null;
        let userMessagePosition = null;
        let editedFromMessageId = null;
        if (user_text.trim()) {
            userMessageId = crypto_1.default.randomUUID();
            ...
            if (attachment_ids.length > 0) {
                ...
                q.set("owner_id", `eq.${owner_id}`);
                q.set("session_id", `eq.${session_id}`);
                q.set("id", `in.(${attachment_ids.map((id) => `"${id}"`).join(",")})`);
        ...
        let fullCode = "";
        let sseBuffer = "";
        const handleAnthropicEvent = (evt) => {
            ...
            if (delta?.type === "text_delta") {
                ...
                if (fullCode) {
                    const streamDuration = extractMotionDurationSeconds(fullText, fullCode, requested_motion_duration);
                    send("code", { html: fullCode, duration_seconds: streamDuration });
                    if (streamDuration)
                        send("duration", { duration_seconds: streamDuration });
                }
            }
        };
        while (true) {
            ...
            for (const part of parts) {
                ...
                if (evt.type === "message_stop") {
                    ...
                    const motionDurationSeconds = extractMotionDurationSeconds(fullText, fullCode, requested_motion_duration);
                    ...
                    const assistantPosition = userMessagePosition !== null
